      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.firstPage}>
          <Text style={styles.h2}>
            The following gamification design document, called <Text style={{ fontFamily: "Sen-Bold", fontSize: 13 }}>{name}</Text> was created using GamiDOC, a tool developed by the
            <Link src="https://www.unitn.it/" style={{ color: "#5555dd", fontFamily: "Sen-Bold", fontSize: 13 }}> <Text > University of Trento </Text></Link>{","}
            <Link src="https://www.fbk.eu/it/" style={{ color: "#5555dd", fontFamily: 'Sen-Bold', fontSize: 13 }}> <Text >Fondazione Bruno Kessler </Text ></Link> {" and "}
            <Link src="https://uwaterloo.ca/games-institute/" style={{ color: "#5555dd", fontFamily: "Sen-Bold", fontSize: 13 }}> <Text >the Games Institute </Text ></Link> {". "}
            {"You can visit the website "}<Link src="https://gami-doc-frontend-test-auth.vercel.app/" style={{ color: "#5555dd", fontFamily: "Sen-Bold", fontSize: 13 }}> <Text>Here </Text></Link>{"."}
            <Br /><Br />
          </Text>
          <Text style={styles.h2}>
        ...
        <View style={{
          ...
          <Text style={styles.h2}>
            ...
            {()=>{ // NON VA BRUTTANEM, apprezzo l'impegno 
                console.log("Dentro")
                numbers = [];
                targetAge.forEach((element)=>{
                  pair = element.split("-")
                  numbers.push(parseInt(pair[0]),parseInt(pair[1]))
                });
                max = Math.max(...numbers);
                min = Math.min(...numbers);
                console.log(max,min)
                return (<Text style={{ fontFamily: "Roboto-Italic", fontSize: 13 }}> {min}-{max} </Text>)
            }}
            </Text> years-old. 
            {(targetUser != "") ? (<Text>Specifically, <Text style={{ fontFamily: "Roboto-Italic", fontSize: 13 }}> {targetUser}</Text></Text>) : ""}
            <Br /><Br />
            {/* Behaviors */}
